...
class MobFactory(ABC):
    @abstractmethod
    def make_mob(self, dungeon: Dungeon, pos_x: int, pos_y: int, level: int, strategy_name: str) -> MobPrototype:
...
class ScifiMobFactory(MobFactory):
    def make_mob(self, dungeon: Dungeon, pos_x: int, pos_y: int, level: int, strategy_name: str) -> MobPrototype:
        match strategy_name:
            ...
            case "passive":
                return Mobs.ToxicMold.make_mob_with_strategy_by_level(dungeon, pos_x, pos_y, level, strategy_name)
            case _:
